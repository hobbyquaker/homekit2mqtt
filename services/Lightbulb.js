    ...
    return function createService_Lightbulb(acc, settings) {
        ...
        if (settings.topic.setBrightness) {
            acc.getService(Service.Lightbulb)
                ...
                .on('set', (value, callback) => {
                    log.debug('< hap set', settings.name || acc.name, 'Brightness', value);
            ...
            if (settings.topic.statusBrightness) {
                mqttSub(settings.topic.statusBrightness, val => {
                    ...
                    log.debug('> hap update', settings.name || acc.name, 'Brightness', brightness);
                    acc.getService(Service.Lightbulb)
                ...
                acc.getService(Service.Lightbulb)
                    ...
                    .on('get', callback => {
                        log.debug('< hap get', settings.name || acc.name, 'Brightness');
                        /* istanbul ignore next */
                        const brightness = Math.round(mqttStatus[settings.topic.statusBrightness] / (settings.payload.brightnessFactor || 1)) || 0;
                        log.debug('> hap re_get', settings.name || acc.name, 'Brightness', brightness);
        ...
        if (settings.topic.setHue) {
            acc.getService(Service.Lightbulb)
                ...
                .on('set', (value, callback) => {
                    log.debug('< hap set', settings.name || acc.name, 'Hue', value);
                    /* istanbul ignore next */
            ...
            if (settings.topic.statusHue) {
                mqttSub(settings.topic.statusHue, val => {
                    ...
                    log.debug('> hap update', settings.name || acc.name, 'Hue', hue);
                    acc.getService(Service.Lightbulb)
                ...
                acc.getService(Service.Lightbulb)
                    ...
                    .on('get', callback => {
                        log.debug('< hap get', settings.name || acc.name, 'Hue');
                        /* istanbul ignore next */
                        const hue = (mqttStatus[settings.topic.statusHue] / (settings.payload.hueFactor || 1)) || 0;
                        log.debug('> hap re_get', settings.name || acc.name, 'Hue', hue);
                        callback(null, hue);
                    });
            }
        }
        ...
        if (settings.topic.setSaturation) {
            acc.getService(Service.Lightbulb)
                ...
                .on('set', (value, callback) => {
                    log.debug('< hap set', settings.name || acc.name, 'Saturation', value);
                    /* istanbul ignore next */
            ...
            if (settings.topic.statusSaturation) {
                mqttSub(settings.topic.statusSaturation, val => {
                    ...
                    log.debug('> hap update', settings.name || acc.name, 'Saturation', sat);
                    acc.getService(Service.Lightbulb)
                ...
                acc.getService(Service.Lightbulb)
                    ...
                    .on('get', callback => {
                        log.debug('< hap get', settings.name || acc.name, 'Saturation');
                        /* istanbul ignore next */
                        const saturation = (mqttStatus[settings.topic.statusSaturation] / (settings.payload.saturationFactor || 1)) || 0;
                        log.debug('> hap re_get', settings.name || acc.name, 'Saturation', saturation);
        ...
        if (settings.topic.setColorTemperature) {
            acc.getService(Service.Lightbulb)
                ...
                .on('set', (value, callback) => {
                    log.debug('< hap set', settings.name || acc.name, 'ColorTemperature', value);
                    const sat = value;
            ...
            if (settings.topic.statusColorTemperature) {
                mqttSub(settings.topic.statusColorTemperature, val => {
                    const sat = val;
                    log.debug('> hap update', settings.name || acc.name, 'ColorTemperature', sat);
                    acc.getService(Service.Lightbulb)
                ...
                acc.getService(Service.Lightbulb)
                    ...
                    .on('get', callback => {
                        log.debug('< hap get', settings.name || acc.name, 'ColorTemperature');
                        /* istanbul ignore next */
                        const saturation = mqttStatus[settings.topic.statusColorTemperature];
                        log.debug('> hap re_get', settings.name || acc.name, 'ColorTemperature', saturation);
                        callback(null, saturation);
                    });
            }
        }
