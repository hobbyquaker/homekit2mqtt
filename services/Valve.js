...
module.exports = function (iface) {
    ...
    return function createService_Valve(acc, settings) {
        ...
        log.debug('> hap set', settings.name || acc.name, 'ValveType', type);
        ...
        if (settings.topic.setDuration) {
            acc.getService(Service.Valve)
                ...
                .on('set', (value, callback) => {
                    log.debug('< hap set', settings.name || acc.name, 'SetDuration', value);
        ...
        if (settings.topic.statusRemainingDuration) {
            ...
            acc.getService(Service.Valve)
                ...
                .on('get', callback => {
                    ...
                    log.debug('> hap re_get', settings.name || acc.name, 'RemainingDuration', inUse);
                    callback(null, inUse);
                });
        }
